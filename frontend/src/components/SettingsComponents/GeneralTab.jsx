...
// import { toggleTheme, updateSetting } from '../../store/slices/navbar/sideMenuSlice';
...
//   const handleToggle = (key, val) => dispatch(updateSetting({ key, value: !val }));
  ...
  return (
    <div className="space-y-12">
      <section>
        <h3 className="text-lg font-bold text-gray-200 mb-8 border-b border-gray-800 pb-4">General notifications</h3>
      ...
      <section>
        <h3 className="text-lg font-bold text-gray-200 mb-8 border-b border-gray-800 pb-4">Language & region</h3>
        <div className="flex items-center justify-between bg-[#0b141a] p-5 rounded-2xl border border-gray-800/50">
          <div>
            ...
            <p className="text-[11px] text-gray-600 mt-1">English (United States)</p>
          </div>
          <div className="flex items-center gap-2 bg-[#142129] px-4 py-2 rounded-xl border border-gray-700">
             <img src="https://flagcdn.com/w20/us.png" alt="us" className="w-5 h-3 object-cover" />
      ...
      <section>
        <h3 className="text-lg font-bold text-gray-200 mb-8 border-b border-gray-800 pb-4">Theme</h3>
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-400">Switch between dark and light appearance</p>
...
const ToggleItem = ({ label, desc, enabled, onToggle }) => (
  <div className="flex items-center justify-between group">
    <div>
      <h5 className="text-sm font-bold text-gray-200 group-hover:text-white transition-colors">{label}</h5>
      <p className="text-[11px] text-gray-600 font-medium">{desc}</p>
    </div>
    <button onClick={onToggle} className={`w-12 h-6 rounded-full relative transition-all duration-300 ${enabled ? 'bg-blue-600 shadow-[0_0_12px_rgba(37,99,235,0.4)]' : 'bg-gray-800'}`}>
